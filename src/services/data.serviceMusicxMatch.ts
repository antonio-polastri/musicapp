...
class DataServiceMusicxMatch implements DataService{
   ...
   //get artist from research
     getArtist  = async (term : string)  =>{
      ...
      //  return await Call.axiosRequestMusix.get('artist?query=' + term).then(response => response.data);
       return    Array<Artist>();
     ...
     getArtistDetails = async (params: string) => {
       ...
       //  return await Call.axiosRequestMusix.get('artist?query=' + params).then(response => response.data);
     ...
     getAlbums  = async (id : string)=>{
      ...
      return    Array<Album>();
  //    return await Call.axiosRequestMusix.get(`release?artist=${id}&type=album|ep`).then(response => response.data);
   ...
   getLyrics  = async (id : string)=>{
      ...
      return await Call.axiosRequestMusix.get(`track.lyrics.get?track_id=${id}${APIKEY}`).then((response) => {
   ...
   getTrackFromLists = async(artistName :string, trackName: string) =>{
      ...
      try {
         ...
         return await Call.axiosRequestMusix.get(`track.search?q_artist=${artistName}&q_track=${trackName}&page_size=1&page=1&s_track_rating=desc${APIKEY}`).then(
            (response) =>{ 

    ...
    return(
      ...
      <li id={item.item._id} className="artist_cont" >
        ...
        <div className="artist_name">
         <h1 title={item.item.name}  onClick={()=>{ item.getAlbums(item.item._id,item.item.origin) ;item.setTitles({artistname :item.item.name,artistId:item.item._id}); item.getWiki(item.item._id) }} > {item.item.name} </h1>
